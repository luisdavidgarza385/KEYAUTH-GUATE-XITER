...
import { Users, Search, Plus, Trash2, Shield, Loader2, Copy, Check, X, ShieldAlert } from "lucide-react";
...
export default function SubResellersPage() {
  ...
  // Checkboxes
  const [subSilent, setSubSilent] = useState(true);
  ...
  function handleCopy(id: string) {
    // Generate a mockup app_id similar to VyperMods screenshot
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    if (subSilent) subscriptions.push("Silent");
    if (subBypass) subscriptions.push("Bypass");
    ...
    <div className="p-6 lg:p-8 space-y-6 max-w-[1400px] mx-auto text-zinc-300">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 border-b border-zinc-800/60 pb-5">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-1 font-medium">Administra a tus vendedores afiliados y sus permisos.</p>
        </div>
      ...
      <div className="relative max-w-md">
        ...
        <input
          ...
          className="w-full bg-zinc-950 border border-zinc-800 rounded-lg pl-10 pr-4 py-2 text-sm text-zinc-200 placeholder:text-zinc-600 outline-none focus:border-emerald-500/50 transition"
        />
      </div>
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-zinc-950 text-zinc-500 text-[10px] font-bold uppercase tracking-wider border-b border-zinc-800/80">
                <tr>
                  ...
                  <th className="px-5 py-3.5">ID Aplicación</th>
                ...
                {filtered.map((sub) => {
                  ...
                  return (
                    <tr key={sub.id} className="hover:bg-zinc-900/40 transition">
                        ...
                        {sub.subscriptions && sub.subscriptions.length > 0 ? (
                          <div className="flex gap-1 flex-wrap">
                            {sub.subscriptions.map((s) => (
                              <span key={s} className="bg-zinc-900 px-1.5 py-0.5 rounded border border-zinc-850 text-zinc-400 text-[10px]">
                                {s}
                              </span>
                            ))}
                          </div>
                        ...
                        {sub.permissions && sub.permissions.length > 0 ? (
                          <div className="flex gap-1 flex-wrap">
                            {sub.permissions.map((p) => (
                              <span key={p} className="bg-zinc-900 px-1.5 py-0.5 rounded border border-zinc-850 text-zinc-500 text-[10px]">
                      ...
                      <td className="px-5 py-4">
                        <span className="inline-flex items-center gap-1 rounded bg-green-950/20 border border-green-900/30 px-2.5 py-0.5 text-[10px] font-bold text-green-400 uppercase">
                          Activo
                        </span>
                      </td>
          ...
          <div className="relative bg-zinc-950 border border-zinc-850 rounded-xl shadow-2xl w-full max-w-lg my-auto overflow-hidden text-zinc-300">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-900 bg-zinc-900/20">
              ...
              <button 
                ...
                className="text-zinc-500 hover:text-zinc-300 p-1 rounded-md hover:bg-zinc-900 transition"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreate} className="px-6 py-5 space-y-4">
              ...
              {/* Subscriptions */}
              <div>
                <label className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 mb-1.5 block">Suscripciones</label>
                <div className="flex gap-4">
                  <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-400 hover:text-zinc-200">
                    <input
                      type="checkbox"
                      checked={subSilent}
                      onChange={(e) => setSubSilent(e.target.checked)}
                  ...
                  <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-400 hover:text-zinc-200">
                    <input
                      type="checkbox"
                      checked={subBypass}
                      onChange={(e) => setSubBypass(e.target.checked)}
                      className="accent-emerald-500 w-4 h-4 rounded cursor-pointer"
                    />
                    <span>Bypass</span>
                  </label>
              ...
              {/* Apps Selection */}
              <div>
                <label className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 mb-1.5 block">Aplicaciones asignadas</label>
                {apps.length === 0 ? (
                  <p className="text-xs text-zinc-600">No hay aplicaciones disponibles</p>
                ) : (
                  <div className="grid grid-cols-2 gap-2 text-sm text-zinc-400">
                    {apps.map((app) => (
                      <label key={app.id} className="flex items-center gap-2 cursor-pointer hover:text-zinc-200">
                        <input
                          type="checkbox"
